...
@router.get(
    ...
    description="Retrieve a list of all orders placed by the "
                "currently authenticated user, sorted by the newest first."
)
...
@router.get(
    ...
    description="Retrieve detailed information about a "
                "specific order. Users can only access their own orders."
)
...
@router.post(
    ...
    description="Create an order from the user's active shopping cart."
                " Validates that the cart is not empty and that the "
                "movies haven't been purchased previously. Clears the cart upon success."
)
async def create_order(
    ...
    check_stmt = select(OrderItemModel.movie_id).join(OrderModel).where(
        OrderModel.user_id == current_user.id,
    ...
    try:
        ...
        return await get_order_by_id(order_id=order.id, db=db, current_user=current_user)
...
@router.patch(
    ...
    description="Cancel a specific order. Only orders in 'pending' status can be canceled."
)
async def cancel_order(
    ...
    if order.status != OrderStatusEnum.PENDING:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"You cannot cancel an order with status '{order.status.value}'."
        )
...
@router.get(
    ...
    description="Retrieve all orders in the system with optional "
                "filtering by user ID, exact status, and date range."
)
