...
async def get_own_cart(
    ...
    return CartSchema(id=cart.id, items=items, total_price=total_price)
...
@router.post(
    ...
    description=(
        "Adds a specific movie to the user's shopping cart."
    ...
    responses={
        400: {
            "description": (
                "Bad Request - Movie is already in the cart," " purchased, or pending."
            ),
            "content": {
                "application/json": {
        ...
        500: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "detail": (
                            "An error occurred while adding" " the movie to your cart."
                        )
                    }
                }
            },
...
async def add_movie_to_cart(
    ...
    if already_ordered:
        raise HTTPException(
            ...
            detail=(
                ...
                " or have a pending order for it."
            ),
        )
    ...
    try:
        ...
        return CartSchema(
            id=cart.id, items=items, total_price=sum(item.movie.price for item in items)
        )
...
async def remove_movie_from_cart(
    ...
    stmt = select(CartItemModel).where(
        CartItemModel.movie_id == movie.id, CartItemModel.cart_id == cart.id
    )
    result = await db.execute(stmt)
    ...
    try:
        ...
        return MessageResponseSchema(
            message="Movie successfully removed from your cart."
        )
    except SQLAlchemyError:
        await db.rollback()
        raise HTTPException(
...
@router.get(
    ...
    description=(
        "Retrieves the shopping cart of a specific user."
...
async def get_user_cart_by_id(
    ...
    stmt = (
        select(CartItemModel)
        .where(CartItemModel.cart_id == user_cart.id)
        .options(selectinload(CartItemModel.movie).selectinload(MovieModel.genres))
    )
    result = await db.execute(stmt)
    items = result.scalars().all()
    ...
    return CartSchema(id=user_cart.id, items=items, total_price=total_price)
