...
def get_accounts_email_notificator(
    ...
    """
    Retrieve an instance of the EmailSenderInterface configured with
    ...
    to send various email notifications (e.g., activation,
    password reset) as required.
...
def get_jwt_auth_manager(
    ...
    This function uses the provided application settings to instantiate
    a JWTAuthManager, which implements the JWTAuthManagerInterface.
...
def get_s3_storage_client(
    ...
    This function instantiates an S3StorageClient using
    the provided settings, which include the S3 endpoint URL,
    access credentials, and the bucket name. The returned client can be used
    to interact with an S3-compatible storage service for file uploads and
    URL generation.
    ...
    Returns:
        S3StorageInterface: An instance of S3StorageClient configured with
        the appropriate S3 storage settings.
    """
