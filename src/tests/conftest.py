...
class DummyEmailSender(EmailSenderInterface):
    ...
    async def send_activation_email(self, email: str, link: str):
        pass

    async def send_activation_complete_email(self, email: str, link: str):
        pass

    async def send_password_reset_email(self, email: str, link: str):
        pass

    async def send_password_reset_complete_email(self, email: str, link: str):
        pass
...
async def clean_database():
    ...
    Ensures complete state isolation between
    tests so they don't affect each other.
    """
    async with TestingSessionLocal() as session:
...
async def client(db_session: AsyncSession) -> AsyncGenerator[AsyncClient, None]:
    ...
    app.dependency_overrides[get_accounts_email_notificator] = (
        lambda: DummyEmailSender()
    )
    app.dependency_overrides[get_s3_storage_client] = lambda: DummyS3Client()

    async with AsyncClient(
        transport=ASGITransport(app=app), base_url=base_api_url
    ) as ac:
        yield ac
...
async def create_test_movie(
    ...
    async def _create_movie(
        name: str = "Interstellar", year: int = 2014, price: float = 14.99
    ):
